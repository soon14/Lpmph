   ...
   $('.check-switch label').click(function(){
    ...
    $('.seller-navL dt').click(function () {
        ...
        if (listcp.hasClass('active')) {
            ...
            listc.height($(this).next('.list-c').find('ul').outerHeight() - 4);
        }
        listcp.toggleClass('active');
    });
    
    //默认展开所有
    $('.seller-navL dt').click();
    
    $('.sellermenu').find('a').click(function(){
    	var menuUrl = $(this).attr('sellermenuurl');
    	$.cookie('menuUrl_sellergzeport', menuUrl, {
			path : '/'
		});
    	window.location.href = GLOBAL.WEBROOT + menuUrl;
    });
    
    var url = $.cookie('menuUrl_sellergzeport');
    if(url!=null&&url!=''){
    	var pmenu = $('#pmenu').text();
    	var smenu = $('#smenu').text();
    	if(smenu==''||pmenu==''){
    		$.eAjax({
    			url : GLOBAL.WEBROOT + '/seller/breakmenu',
    			data :  {'menuUrl':url},
    			datatype:'json',
    			success : function(returnInfo) {
    			
    				$('#pmenu').html(returnInfo.pMenuTitle);
    				$('#smenu').html(returnInfo.menuTitle);
    			}
    		});
    	}
    }

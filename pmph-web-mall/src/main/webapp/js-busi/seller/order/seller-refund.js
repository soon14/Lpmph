function addLoadParamsToUrl(_load_url_) {
	...
	var isCompensate = $('select[name="isCompensate"]').val();
	if (isCompensate != "" && isCompensate != undefined) {
		_load_url_ += '&isCompensate=' + isCompensate;
	}
...
$(function() {
	...
	var pInit = function() {
		...
		var init = function() {
			//待处理
            $('#refundTodo').click(function(){
            	var s = '';
            	var _load_url_ = '/seller/order/refund/querytodo?ls='+s;
            	_load_url_ = addLoadParamsToUrl(_load_url_);
            	$("#btnPrint").hide();
            	$(this).attr("class","active");
            	$('#refundHandled').attr("class","");
            	$('#order-contentDiv').load(GLOBAL.WEBROOT + _load_url_);
            });
	    	
	    	//已处理
            $('#refundHandled').click(function(){
            	var s = '';
            	var _load_url_ = '/seller/order/refund/queryhandled?ls='+s;
            	_load_url_ = addLoadParamsToUrl(_load_url_);
            	$("#btnPrint").show();
            	$(this).attr("class","active");
            	$('#refundTodo').attr("class","");
                $('#order-contentDiv').load(GLOBAL.WEBROOT + _load_url_);
		    ...
		    $('#btnPrint').click(function(){
		    	...
		    	$("#searchForm").attr("action",GLOBAL.WEBROOT + '/seller/order/refund/printList');
		    	$("#searchForm").submit();
			});

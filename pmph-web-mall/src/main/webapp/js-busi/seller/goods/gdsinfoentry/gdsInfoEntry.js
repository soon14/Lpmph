...
$(function(){
	...
	$("#gdsCombine").live('click',function(){
		...
		bDialog.open({
            ...
            url : GLOBAL.WEBROOT + "/seller/goods/gdsinfoentry/togdscombine",
...
//		}
    ...
    $("#backGdsManage").click(function(){
    	window.location.href =  GLOBAL.WEBROOT +"/seller/goods/gdsmanage?shopId="+$("#shopId").val();
    });
		...
		buttonOpen : function(str,button){
			if('senior-price' == button){
				if('open' == str){
					...
					$("#open-ladder-price").html('<span class="sbtn sbtn-bluw required priceLength priceNumber gjpri" value="[]" id="productPrice" onclick="openPriceWin(this)">高级价格</span>');
				}else{
					price_switch = "0";
				...
				if('open' == str){
					...
					$("#open-ladder-stock").html('<span class="sbtn sbtn-bluw required digits gjkc" value="[]" id="productStock" onclick="openStockWin(this)">高级库存</span>');
				}else{
					stock_switch = "0";
...
//	    	}
	    ...
	    openStockWin : function(obj){
    	        bDialog.open({
    	            ...
    	            width : 680,
    	            height : 400,
    	            url :  GLOBAL.WEBROOT + "/seller/goods/gdsinfoentry/toseniorstock?shopId="+$("#shopId").val()+"&&editFlag='0'",
    	           // url :  GLOBAL.WEBROOT + "/seller/goods/gdsinfoentry/toseniorstock?shopId=69&&editFlag='0'",
    	            params : {
	    ...
	    openPriceWin : function(obj){
    	        bDialog.open({
    	            ...
    	            url : GLOBAL.WEBROOT + "/seller/goods/gdsinfoentry/toseniorprice?shopId="+$("#shopId").val(),
    	            callback:function(data){
    	            	if(data && data.results && data.results.length > 0 ){
    						$(obj).attr('value',data.results[0].param);
    					}
	    ...
	    openSkuPictrueWindow : function(obj){
	        bDialog.open({
	            ...
	            url : GLOBAL.WEBROOT + "/seller/goods/gdsinfoentry/toskupictrues?shopId="+$("#shopId").val(),
	            callback:function(data){
	            	if(data && data.results && data.results.length > 0 ){
						$(obj).attr('value',data.results[0].param);
					}
...
//	    	}
		...
		saveGdsInfo : function(obj){
			...
			$.eAjax({
				url : GLOBAL.WEBROOT + "/seller/goods/gdsinfoentry/savegdsinfo",
				data :param,

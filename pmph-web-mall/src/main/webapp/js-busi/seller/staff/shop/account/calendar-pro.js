	...
	function getIndexDay(){
		...
		for( var k = 0;k<calendarDate.days;k++ ){
			...
			for( var d in calendarDate.opt.data ){
				...
				if( flag ){
					var cur=calendarDate.opt.data[d].data.split(",");
					if (cur.length==2) {
						riliHtml += '<div class="ht-rili-td ht-rili-onclick" data-date="'+ dataDateStr +'"><span class="ht-rili-day">'+ (k + 1) +'</span>'+
						'<span class="ht-rili-money" data-money="'+ calendarDate.opt.data[d].data +'">'+ cur[0] +'</span><br/>'+
						'<span class="ht-rili-money zhichu-money" data-money="'+ calendarDate.opt.data[d].data +'">'+cur[1]+'</span>'+
						'</div>';
						break;
					}else if(cur.length==1){
						riliHtml += '<div class="ht-rili-td ht-rili-onclick" data-date="'+ dataDateStr +'"><span class="ht-rili-day">'+ (k + 1) +'</span><span class="ht-rili-money" data-money="'+ calendarDate.opt.data[d].data +'">'+ cur[0] +'</span></div>';
						break;
					}
	...
	$.fn.extend({
	    calendar:function(opt){
	        if( (opt.beginDate != undefined && opt.endDate != undefined )||( opt.data.length > 0 ) ){
	        	...
	        	var date = new Date();
	        	beginDate = (date.getYear()+1900)+"-"+(date.getMonth())+"-"+date.getDay();
				...
				calendarDate.month = parseInt(beginDate.split('-')[1])+1;//设置起始日期为当前日期
	    	...
	    	$('.ht-rili-leftarr').on('click',function(){
	    		...
	    		$.eAjax({
	      			...
	      			async : true,
	      			data : {shopId : $("#shopId").val(),year:calendarDate.year,month:calendarDate.month},
	    			type : "post",
	      			dataType:'json',

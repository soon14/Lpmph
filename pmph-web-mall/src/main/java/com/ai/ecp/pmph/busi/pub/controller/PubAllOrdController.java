...
@Controller
@RequestMapping(value = "/pubord/all")
public class PubAllOrdController extends EcpBaseController {
    ...
    public String init(Model model,PubAllOrdVO vo) throws Exception {
    	...
    	String status = PubOrdConstants.CustomerRequestStatus.REQUEST_STATUS_ALL;
        ...
        return "/pub/order/all-list";
    }
    
    @RequestMapping(value = "/checkpubuser")
    public EcpBaseResponseVO checkpubuser(){
    	EcpBaseResponseVO vo = new EcpBaseResponseVO();
    	@SuppressWarnings("rawtypes")
		BaseStaff staff = new BaseInfo().getStaff();
    	PubUserInfoReqDTO pubUserInfoReqDTO = new PubUserInfoReqDTO();
    	pubUserInfoReqDTO.setStaffCode(staff.getStaffCode());
    	Boolean flag = pubRSV.isPubUser(pubUserInfoReqDTO);
    	if(!flag){

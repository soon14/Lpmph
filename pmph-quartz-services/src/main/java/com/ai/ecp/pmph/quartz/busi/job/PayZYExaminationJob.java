...
public class PayZYExaminationJob extends AbstractCommonQuartzJob {
    
    private static final String MODULE = PayZYExaminationJob.class.getName();
    ...
    protected void doJob(Map<String, String> extendParams) throws JobExecutionException {
        LogUtil.info(MODULE, "=============begin PayZYExaminationJob job =============");
        try{
            ...
            List<RPayQuartzInfoResponse> list = payQuartzInfoForZYOrdRSV.queryNotDealZYExaminationOrder(payQuartzInfoRequest);
            if(list!=null&&!list.isEmpty()){
                for(RPayQuartzInfoResponse quartzInfoResponse:list){
                    try{
                        ...
                        payQuartzInfoForZYOrdRSV.dealZYExaminationOrder(rPayQuartzInfoRequest);
                    }catch(Exception e){
                        LogUtil.error(MODULE,"=============订单"+quartzInfoResponse.getId()+"处理泽元考试网授权接口异常=============",e);
                    }
                }
            }
        } catch(Exception err){
            LogUtil.error(MODULE,"=============PayZYExaminationJob error =============",err);
        }
        LogUtil.info(MODULE,"=============end PayZYExaminationJob job =============");
    }

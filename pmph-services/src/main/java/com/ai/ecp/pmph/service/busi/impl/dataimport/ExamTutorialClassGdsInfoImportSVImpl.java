...
import com.ai.ecp.pmph.service.busi.interfaces.dataimport.IExamTutorialClassGdsInfoImportSV;
 ...
 * @since JDK 1.6
 */
public class ExamTutorialClassGdsInfoImportSVImpl extends BaseGdsInfoImportSV implements IExamTutorialClassGdsInfoImportSV {
...
//        }
    ...
    protected String getSrcGdsCodePrefix() throws BusinessException {
        return PmphGdsDataImportConstants.CodePreFix.ZY_EXAM_TUTORIALCLASS_ORIGN_CODE_PREFIX;
    }
...
//    }
    ...
    protected String getGdsStatus(Map map, List<String> list) throws BusinessException {
        ...
        boolean flag = true;
        if (this.isNotNull_(map,"FixedPrice")) {
            zhipindingjia = (long) (Float.parseFloat(this.getValue_(map,"FixedPrice")) * 100);
        }
        if (zhipindingjia <= 0) {
            flag = false;
        }
...
//        gdsInfoReqDTO.setGdsStatus(GdsConstants.GdsInfo.GDS_STATUS_ONSHELVES);
        ...
        Long tpPrice = 0L;
        if (this.isNotNull_(map,"FixedPrice")) {
            tpPrice = (long) (Float.parseFloat(this.getValue_(map,"FixedPrice")) * 100);
        }
    ...
    private List<String[]> createAddPropInfoList(Map map) throws BusinessException {
        ...
        //总时长（以小时为单位，取整）
        if (this.isNotNull_(map,"Length")) {
            propInfoList.add(new String[] { "1033", this.getValue_(map,"Length") });
        }else{
            propInfoList.add(new String[] { "1033",""});
        }
        
        //课时数
        if (this.isNotNull_(map,"Cnt1")) {
            propInfoList.add(new String[] { "1034", this.getValue_(map,"Cnt1") });
        }else{
            propInfoList.add(new String[] { "1034",""});
        }
        
        //同步练习数
        if (this.isNotNull_(map,"Cnt2")) {
            propInfoList.add(new String[] { "1035", this.getValue_(map,"Cnt2") });
        }else{
            propInfoList.add(new String[] { "1035",""});
        }
        
        //模拟试卷数
        if (this.isNotNull_(map,"Cnt3")) {
            propInfoList.add(new String[] { "1036", this.getValue_(map,"Cnt3") });
        }else{
            propInfoList.add(new String[] { "1036",""});
        }
        
        //考试网商品类型标识属性
        propInfoList.add(new String[] { "1040", PmphGdsDataImportConstants.Commons.ZY_EXAM_PROP_TUTORIALCLASS});
    ...
    private GdsInfoReqDTO addCategoryInfo(Map map, GdsInfoReqDTO gdsInfoReqDTO)
        ...
        if(StringUtils.isBlank(ecpCatgs)){
        	//设置成辅导班的其它分类
        	ecpCatgs = PmphGdsDataImportConstants.Commons.CATEGORY_EXAM_TUTORIALCLASS_OTHER_ID;
        }
...
//        gdsInfoReqDTO.setGdsStatus(GdsConstants.GdsInfo.GDS_STATUS_ONSHELVES);
        ...
        if (!list.contains("gdsPrice") && this.isNotNull_(map,"FixedPrice")) {
...
//            gdsSku2PriceReqDTO.setPriceTypeCode(GdsConstants.GdsInfo.SKU_PRICE_TYPE_ORDINARY);
//            GdsPriceReqDTO gdsPriceReqDTO = new GdsPriceReqDTO();
            Long tpPrice = (long) (Float.parseFloat(this.getValue_(map,"FixedPrice")) * 100);
//            gdsPriceReqDTO.setPrice(tpPrice);
//            gdsSku2PriceReqDTO.setPrice(gdsPriceReqDTO);
//            sku2PriceReqDTOs.add(gdsSku2PriceReqDTO);
//

...
import com.ai.ecp.general.order.interfaces.IPayQuartzInfoRSV;
...
public class PayZYExaminationSVImpl implements IPayZYExaminationSV {
    ...
    private static final String MODULE = PayZYExaminationSVImpl.class.getName();
    ...
    public void joinTransaction(JSONObject message, TransactionStatus status, String transactionName) {
        try {
            ...
            LogUtil.info(MODULE,"PayZYExaminationSVImpl============="+paySuccInfo.toString());
            LogUtil.info(MODULE,"处理泽元考试网开始，订单号："+paySuccInfo.getOrderId());
            ...
            LogUtil.info(MODULE,"处理泽元考试网结束，订单号："+paySuccInfo.getOrderId());
        } catch (BusinessException be) {
            LogUtil.error(MODULE, "支付回调泽元考试网授权接口处理异常", be);
            be.printStackTrace();
            status.setRollbackOnly();
            throw new BusinessException(be.getErrorCode());
        } catch (Exception e) {
            LogUtil.error(MODULE, "支付回调泽元考试网授权接口处理异常", e);
            status.setRollbackOnly();
    ...
    public void deal(PaySuccInfo paySuccInfo) {
        try{
            LogUtil.info(MODULE,"处理泽元考试网开始============="+paySuccInfo.toString());
            LogUtil.info(MODULE,"处理泽元考试网开始，子订单号："+paySuccInfo.getSubOrder());
            //获取定时任务表数据
            ...
            payQuartzInfoRequest.setTaskType(PayStatus.PAY_TASK_TYPE_04);
            ...
            String catgCode = "1401";
            gds2CatgReqDTO.setCatgCode(catgCode);
            ...
            if(flag){
                ...
                //goodsType
                if("1401".equals(catgCode)){
                    goodsType = "FDB";
                }else{
                    //判断试卷或是试卷包
                    try{
                        GdsInfoReqDTO gdsInfo = new GdsInfoReqDTO();
                        gdsInfo.setId(ordSub.getGdsId());
                        gdsInfo.setGdsQueryOptions(new GdsQueryOption[]{GdsQueryOption.BASIC,GdsQueryOption.PROP});
                        GdsInfoRespDTO gdsInfoRespDTO = gdsInfoQueryRSV.queryGdsInfoByOption(gdsInfo);
                        
                        if(gdsInfoRespDTO!=null){
                            Map<String, GdsPropRespDTO> map = gdsInfoRespDTO.getAllPropMaps();
                            if(map!=null){
                                GdsPropRespDTO gdsPropRespDTO = map.get(1040);
                                List<GdsPropValueRespDTO> list = gdsPropRespDTO.getValues();
                                if(list!=null&&!list.isEmpty()){
                                    goodsType = list.get(0).getPropValue();
                                }
                        ...
                        LogUtil.error(MODULE, "泽元数字教材授权goodsType异常", e);
                    }
                    
                }
                
                //authUrl
                BaseSysCfgRespDTO sysDTO=SysCfgUtil.fetchSysCfg(PayStatus.PAY_ZYEXAMINATION_URL);
                authUrl = sysDTO.getParaValue();
                
                //处理
                RPayQuartzInfoRequest payQuartzInfoDTO = new RPayQuartzInfoRequest();
                payQuartzInfoDTO.setTaskType(PayStatus.PAY_TASK_TYPE_04);
                ...
                if(i==1){
                    ...
                    try{
                        ...
                        authRequest.setGoodsType(goodsType);
                        authRequest.setTime(Time);
                        ...
                        String responseMge = "";
                        try{
                            response = aipZYAuthRSV.sendExaminationRequest(authRequest);
                        }catch(Exception e){
                            LogUtil.error(MODULE, "泽元考试网授权aip接口异常", e);
                        ...
                        if(AipZYAuthRequest._ZVING_STATUS_OK.equals(response.getStatus())){
                            ...
                            rConfirmDeliveRequest.setRemark("泽元考试网授权");
                        ...
                        payIntfReqLogSV.addPayZYExaminationLog(log);
                    }catch(Exception e){
                        LogUtil.error(MODULE, "泽元考试网授权接口处理异常", e);
                    }
            ...
            if(updateToUndo){
              //把正在处理中的状态改到未处理
                ...
                payQuartzInfoRequest2.setTaskType(PayStatus.PAY_TASK_TYPE_04);
            ...
            //TODO
            LogUtil.info(MODULE,"处理泽元考试网结束，子订单号："+paySuccInfo.getSubOrder());
        }catch(Exception e){
            LogUtil.error(MODULE, "泽元考试网授权接口处理异常,子订单："+paySuccInfo.getSubOrder(), e);
            throw e;
        }
    }

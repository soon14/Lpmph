...
function LoadScroll(element, options) {
    ...
    this.renderList = function (page, type) {
        ...
        var pageNumber = page;
        var url=this.URL + page +"&pageNumber="+pageNumber;
	...
	this.refreshParams = function(params){
		defaults.params = $.extend(defaults.params,params);
	}
    this.init();
};
/* 搜索 begin*/
$(function () {
    ...
    if (needSerInput.size() > 0) {
        var searchPageWrap = $('<div id="searchPage" class="ui-page-contanier">' +
            ...
            '<li><i class="iconfont icon-pro"></i> 商品</li>' +
            '<li><i class="iconfont icon-shop"></i> 店铺</li>' +
            '</ul>' +
            '</div>' +
            '</div>' +

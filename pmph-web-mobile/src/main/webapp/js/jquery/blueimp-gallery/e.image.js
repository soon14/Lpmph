...
;(function(w) {
    ...
    _template += '<a class="close">&times;</a>';
    ...
    var eImage = {
         ...
         *
         */
        gallery : function(box,opt){
            var opt= $.extend({
                winBody:$('body'),
                bigImg:'',
                ignoreImg:''
            },opt);
            var winBody = opt.winBody,controlBar = null;
            if($('#eImageControlBar',$(winBody)).size() > 0){
                controlBar = $('#eImageControlBar',$(winBody));
            }else{
                var tmp = _template;
                var controlBar = $(tmp);
                controlBar.removeClass('blueimp-gallery-carousel');
                winBody.append(controlBar);
            }
            $(box).click(function(event) {
                event = event || window.event;
                var target = event.target || event.srcElement,
                    link = target.src ? target.parentNode : target,curIndex=$(target).index();
                if(target.tagName !='IMG' || $(target).hasClass(opt.ignoreImg)){
                    return;
                };
                $(opt.bigImg,$(this)).each(function(index){
                  if($(this).is($(target))){
                      curIndex=index;
                  }
                });

...
$(function(){
	...
	$('#allow').click(function(){
		...
		var reviewUrl = null;
		if(applyType == "0"){
			reviewUrl = GLOBAL.WEBROOT+'/refundReview/confirmReview';
		} else if(applyType == "1"){
			reviewUrl = GLOBAL.WEBROOT+'/backReview/confirmReview';
		} else {
			return ;
		}
		$.eAjax({
					...
					eDialog.alert('审核成功',function(){
						if(applyType == "0"){
							if(status == "00"){ //一级审核
								...
								window.location.href = GLOBAL.WEBROOT+'/refundReview/refund2';
							} else if(status == "07") { //三级审核
								window.location.href = GLOBAL.WEBROOT+'/refundReview/refund3';
							}

						} else if(applyType == "1"){

							if(status == "00"){ //一级审核
								window.location.href = GLOBAL.WEBROOT+'/backReview/backgds1';
							} else if(status =="06"){ //二级审核
								window.location.href = GLOBAL.WEBROOT+'/backReview/backgds2';
							} else if(status == "07") { //三级审核
								window.location.href = GLOBAL.WEBROOT+'/backReview/backgds3';
							}
						}
...
//						window.location.href = GLOBAL.WEBROOT+'/ordback/review';
	...
	$('#update_money_btn').click(function(){
		...
		bDialog.open({
		    ...
		    height : 300,
		    scroll : false,
		    url : GLOBAL.WEBROOT+'/backReview/goModifyMoney?orderId=' + ordId+ '&scale=' + scale+ '&backId=' + backId+ '&backMoney=' + backMoney+ '&reduCulateScore=' + reduCulateScore+ '&staffId=' + staffId+ '&lastFlag=' + isLastFlag,

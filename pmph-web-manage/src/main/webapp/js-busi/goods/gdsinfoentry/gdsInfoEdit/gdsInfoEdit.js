...
$(function() {
	...
	$("#topSave").click(function() {
		if($("#copyFlag").val()!='1' && ($("#GDS_STATUS").val()=="11" || $("#GDS_STATUS").val()=="33")&& $("#GDS_VERIFY_SWITCH").val()=="1"){
			$("#saveGdsInfoShelved").trigger('click');
		}else if($("#copyFlag").val()!='1' && ($("#GDS_STATUS").val()=="11" || $("#GDS_STATUS").val()=="33")&& $("#GDS_VERIFY_SWITCH").val()!="1"){
			$("#saveGdsInfoShelvedWithoutVerify").trigger('click');
		}else{
	...
	//（已上架）
	$("#saveGdsInfoShelved").click(function() {
		if (!$("#baseInfoForm").valid()) {
			$body = (window.opera) ? (document.compatMode == "CSS1Compat"
					? $('html')
					: $('body')) : $('html,body');
		...
		GdsInfoEntry.saveGdsInfoShelved();
	});
	//提交更新审核
	$("#submitUpdateReview").click(function() {
		...
		GdsInfoEntry.saveAndSubmitUpdateReview();
	});
	//直接保存更新线下+线上信息
	$("#saveGdsInfoShelvedWithoutVerify").click(function() {
		if (!$("#baseInfoForm").valid()) {
			$body = (window.opera) ? (document.compatMode == "CSS1Compat"
					? $('html')
					: $('body')) : $('html,body');
		...
		GdsInfoEntry.saveGdsInfoShelvedWithoutVerify();
	});

	$("#backGdsManage").click(function() {
		if($("#gdsVerifyFlag").val()=='1'){
			//如果是商品审核开关
			window.location.href = GLOBAL.WEBROOT + "/gdsverify?shopId="
			+ $("#shopId").val();
		}else{
			window.location.href = GLOBAL.WEBROOT + "/gdsmanage?shopId="
			+ $("#shopId").val();
		}
	...
	$(".gdsVerify").live('click',function(e){
		...
		var params = {};
			...
			$.eAjax({
				url : GLOBAL.WEBROOT + "/gdsverify/verifygds",
				data : params,
...
var GdsInfoEntry = {
	...
	openStockWin : function(obj) {
		...
		bDialog.open({
					...
					height : 400,
					url : GLOBAL.WEBROOT
							+ "/gdsinfoentry/toseniorstock?shopId="
	...
	openPriceWin : function(obj) {
		bDialog.open({
					...
					url : GLOBAL.WEBROOT
							+ "/gdsinfoentry/toseniorprice?shopId="
							+ $("#shopId").val(),
					callback : function(data) {
						if (data && data.results && data.results.length > 0) {
							$(obj).attr('value', data.results[0].param);
	...
	openSkuPictrueWindow : function(obj) {
		bDialog.open({
					...
					url : GLOBAL.WEBROOT
							+ "/gdsinfoentry/toskupictrues?shopId="
							+ $("#shopId").val(),
					callback : function(data) {
						if (data && data.results && data.results.length > 0) {
							$(obj).attr('value', data.results[0].param);
...
//	    	}
	...
	saveGdsInfo : function() {
		...
		$.eAjax({
					url : GLOBAL.WEBROOT + "/gdsinfoentry/editgdsinfo",
					data : param,
					success : function(returnInfo) {
						$.gridUnLoading();
						var _successMsg = "";
	...
	// 编辑保存商品并提交更新审核(已上架)
	saveAndSubmitUpdateReview : function() {
		if (GdsInfoEntry.initSaveGdsInfoParam() == "") {
			return;
		}
		var param = GdsInfoEntry.initSaveGdsInfoParam();
			...
			$.eAjax({
				...
				success : function(returnInfo) {
					...
					if ($("#copyFlag").val() == "1") {
						...
						_successMsg = "保存且提交审核成功！";
						_errorMsg = "编辑保存失败！";
						_errorMsg2 = "保存成功但未提交审核！";
					}
					if (returnInfo.ecpBaseResponseVO.resultFlag == 'ok') {
						var params = {};
						params.operateType = '33';
						params.gdsId = param.id;
						params.shopId = param.shopId;
						$.eAjax({
							url : GLOBAL.WEBROOT + "/gdsmanage/commitforverifyShelved",
							data : params,
	...
	//直接保存线下+线上商品信息
	saveGdsInfoShelvedWithoutVerify : function(){
			...
			$.eAjax({
				...
				success : function(returnInfo) {
					...
					if ($("#copyFlag").val() == "1") {
						...
						_successMsg = "保存且上架更新成功！";
						_errorMsg = "编辑保存失败！";
						_errorMsg2 = "保存成功但上架更新失败！";
					}
					if (returnInfo.ecpBaseResponseVO.resultFlag == 'ok') {
						eDialog.success(_successMsg, {
							buttons : [{
	...
	initSaveGdsInfoParam : function() {
		...
		param.ext1 = $("#EXT1").val();
		//商品状态
		param.gdsStatus = $("#GDS_STATUS").val();
		return param;
	},
	// 获取阶梯价
	...
	validatorSku : function(obj, str) {
		...
		}else if(skuPriceObj.attr('name')=='appPrice_param'){
			if(value !="" && !_price.test(value)){
				skuPriceObj.nextAll().remove();
				skuPriceObj.parent().append("</br><b name='error' style='color:#C62626'>手机专享价格式不合法</b>");
			}else{
				skuPriceObj.nextAll().remove();
			}
		}
